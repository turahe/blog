...
export default function PlayMusic({ musicFile }: PlayMusicProps) {
  ...
  return (
    <div className="mb-6">
      ...
      <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg p-4 max-w-md mx-auto`}>
        {/* Control Buttons */}
        <div className="flex items-center justify-center space-x-2">
          ...
          <div className="flex-1 mx-2">
            <div className="flex items-center space-x-2">
              <span className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{formatTime(currentTime)}</span>
              <input
                type="range"
                min="0"
                max="100"
                ...
                className={`flex-1 h-2 rounded-lg appearance-none cursor-pointer slider ${isDark ? 'bg-gray-600' : 'bg-gray-200'}`}
                style={{
                  background: `linear-gradient(to right, #3b82f6 0%, #3b82f6 ${progress}%, ${isDark ? '#4b5563' : '#e5e7eb'} ${progress}%, ${isDark ? '#4b5563' : '#e5e7eb'} 100%)`
                }}
              />
              <span className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{formatTime(duration)}</span>
            </div>
          </div>
